...
| **strategy_notes** | ORE manual-review flag on `OutreachRecommendation`. Type: `dict | None`. Set **only** when the draft and the critic fallback both fail (manual-review path). When set, shape is `{"message": "Manual Review: …"}` with a short summary of critic violations (e.g. "Manual Review: Missing opt-out language"). When the critic passes (or fallback passes), `strategy_notes` remains `None`. Used for auditing and UI to show that the stored draft needs human review. See [signal-models.md](signal-models.md) §1.4 and [Outreach-Recommendation-Engine-ORE-design-spec.md](Outreach-Recommendation-Engine-ORE-design-spec.md) (Issue #120 M3). |
